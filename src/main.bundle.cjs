...
var require_node = __commonJS({
  "node_modules/debug/src/node.js"(exports2, module2) {
    ...
    function log(...args2) {
      return process.stderr.write(util3.formatWithOptions(exports2.inspectOpts, ...args2) + "\n");
    }
    function save(namespaces) {
      if (namespaces) {
...
var IS_RENDER = !!process.env.RENDER;
async function getBrowser() {
  ...
  const browser = await chromium.launch({
    headless: IS_RENDER,
    // false in local, true in Render
    executablePath: chromiumExecutable,
    args: IS_RENDER ? ["--no-sandbox", "--disable-setuid-sandbox"] : []
  });
  const context = await browser.newContext(
    hasStorage ? { storageState: storagePath } : {}
  );
  const page = await context.newPage();
  await page.goto("https://predict.sparklane.fr", { timeout: 45e3, waitUntil: "domcontentloaded" });
  if (!IS_RENDER && !hasStorage) {
  ...
  if (!IS_RENDER && import_fs.default.existsSync(storagePath)) {
    ...
    if (currentUrl.includes("/login") || currentUrl.includes("/auth") || currentUrl.includes("sso") || !currentUrl.includes("predict.sparklane.fr")) {
      console.log("\u274C Session expir\xE9e ou redirection d\xE9tect\xE9e -> reconnecte-toi compl\xE9tement dans le navigateur, puis appuie sur ENTER ici.");
      await waitForEnter();
      await context.storageState({ path: storagePath });
...
async function goToTargetList(page, selectors2) {
  ...
  await page.waitForSelector(selectors2.menuLists, { timeout: 15e3 });
  await page.click(selectors2.menuLists);
  console.log("\u{1F4CB} \xC9tape 2: Trouver la liste cible...");
  try {
    await page.waitForSelector(selectors2.targetListName, { timeout: 15e3 });
    await page.click(selectors2.targetListName);
  } catch (e) {
    const listName = selectors2.targetListName.replace("text=", "").trim();
    console.log(`   -> S\xE9lecteur direct \xE9chou\xE9, recherche par texte: "${listName}"`);
    const listEl = page.getByText(listName, { exact: true }).first();
    await listEl.waitFor({ timeout: 1e4 });
    await listEl.click();
  }
  console.log("\u23F3 \xC9tape 3: Attente chargement des entreprises...");
  try {
    await page.waitForSelector(selectors2.companyCard, { timeout: 2e4 });
    console.log("\u2705 Navigation vers la liste r\xE9ussie !");
  } catch (e) {
    console.error("\u274C Timeout : aucune carte entreprise d\xE9tect\xE9e. V\xE9rifie le s\xE9lecteur companyCard :", selectors2.companyCard);
    throw e;
  }
}
async function getTargetListId(page) {
  console.log("\u{1F50D} R\xE9cup\xE9ration de l'ID de la liste pour l'API...");
  await page.waitForTimeout(2e3);
  const url2 = page.url();
  const match = url2.match(/\/companies-list\/(\d+)/);
  if (match && match[1]) {
    console.log(`\u2705 ID de la liste trouv\xE9 : ${match[1]}`);
    return match[1];
  }
  try {
    const urlParams = new URL(url2).searchParams;
...
async function interceptToken(page) {
  console.log("\u{1F575}\uFE0F Recherche du token d'API Sparklane (God Mode)...");
  try {
    ...
    if (token) {
      console.log("\u{1F525} Token API extrait du stockage local avec succ\xE8s !");
      return token;
    }
  } catch (e) {
    console.log("\u274C Erreur pendant l'extraction du token du navigateur :", e.message);
  }
  console.log("\u26A0\uFE0F Token non trouv\xE9 dans le stockage. Tentative d'interception r\xE9seau experte...");
  return new Promise(async (resolve) => {
    ...
    const requestListener = (request) => {
      ...
      if (url2.includes("/api/")) {
        ...
        if (headers["authorization"] && headers["authorization"].startsWith("Bearer ") && !tokenFound) {
          tokenFound = true;
          console.log("\u{1F525} Token API intercept\xE9 via le r\xE9seau avec succ\xE8s !");
    ...
    console.log("\u{1F300} Rechargement de la page pour forcer l'envoi de requ\xEAtes API...");
    page.reload().catch(() => {
    });
    setTimeout(() => {
      if (!tokenFound) {
        console.log("\u26A0\uFE0F Impossible de trouver le token apr\xE8s 90 secondes.");
        page.off("request", requestListener);
        resolve(null);
      }
    }, 9e4);
...
async function fetchCompaniesAPI(page, listId, pageSize = 1e3) {
  console.log(`\u{1F4E1} \xC9coute du r\xE9seau : Extraction des entreprises (Liste ${listId})`);
  return new Promise(async (resolve) => {
    let listFound = false;
    const timeoutId = setTimeout(() => {
      if (!listFound) {
        console.log("\u26A0\uFE0F Timeout r\xE9seau : La r\xE9ponse de la liste n'a pas \xE9t\xE9 trouv\xE9e.");
        resolve(null);
      }
    }, 3e4);
    try {
      const response = await page.waitForResponse((response2) => {
        const url2 = response2.url();
        return url2.includes("/api/") && (url2.includes("companies") || url2.includes("company")) && parseInt(response2.status()) === 200 && response2.request().method() === "POST";
      }, { timeout: 25e3 });
      listFound = true;
      clearTimeout(timeoutId);
      console.log("\u{1F525} Donn\xE9es d'entreprises intercept\xE9es avec succ\xE8s !");
...
async function fetchContactsAPI(token, companyUniqueId, companyNumber, page = null) {
  const url2 = `https://predict.sparklane.fr/api/company-search/datalake-contacts-searches`;
    ...
    if (page) {
      ...
      while (retries > 0) {
        try {
          ...
          }, { fetchUrl: url2, fetchToken: token, fetchPayload: payload });
          ...
          if (e.message.includes("Execution context was destroyed")) {
            ...
            if (!page.isReloadingToken) {
              ...
              console.log(`\u26A0\uFE0F Token expir\xE9 (401). Rechargement de la page pour rafra\xEEchir la session...`);
              await page.evaluate(() => {
                window.__cachedSparklaneToken = null;
              }).catch(() => {
              });
              ...
              if (currentUrl.includes("login") || currentUrl.includes("auth") || currentUrl.includes("sign")) {
                console.log("\n" + "=".repeat(60));
                console.log("\u{1F6A8}  ALERTE : SESSION SPARKLANE EXPIR\xC9E (Temps \xE9coul\xE9)  \u{1F6A8}");
                console.log("=".repeat(60));
                console.log("\u{1F449} Le bot est PAUS\xC9 pour \xE9viter de fausser les donn\xE9es.");
                console.log("\u{1F449} VEUILLEZ AFFICHER LA FENETRE DU NAVIGATEUR ET VOUS RECONNECTER MANUELLEMENT.");
                ...
                console.log("\n\u2705 Reconnexion d\xE9tect\xE9e ! Reprise de l'extraction en cours...");
                await new Promise((r) => setTimeout(r, 5e3));
              }
              page.isReloadingToken = false;
            } else {
              console.log(`\u231B Attente (401) : Un autre worker recharge d\xE9j\xE0 la page pour le token...`);
              while (page.isReloadingToken) {
                await new Promise((r) => setTimeout(r, 1e3));
              }
              await new Promise((r) => setTimeout(r, Math.random() * 2e3));
            }
            retries--;
          } else {
            throw e;
...
async function processCompanies(page, baseUrl, targetListId, dryRun2 = false, selectors2, listName = "DEFAULT_LIST", turbo2 = false) {
  const CONCURRENCY = turbo2 ? 3 : 1;
  console.log(`ETAPE 4 : EXTRACTION (${turbo2 ? `TURBO x${CONCURRENCY}` : "Normal"}) - Liste: ${listName}`);
  const csvDir = "data/lists";
  if (!import_fs2.default.existsSync(csvDir)) import_fs2.default.mkdirSync(csvDir, { recursive: true });
  ...
  if (turbo2) {
    console.log("\u{1F680} TURBO: blocage ressources + " + CONCURRENCY + " workers parall\xE8les");
  ...
  if (import_fs2.default.existsSync(csvFile)) {
    ...
    console.log(`\u{1F4C2} Reprise : ${dbCount} entreprises d\xE9j\xE0 dans le CSV.`);
  }
  let totalInList = Infinity;
  try {
    const ct = await page.locator(selectors2.companiesCounter).textContent({ timeout: 3e3 });
    const m = ct.match(/(\d+)\s*\/\s*(\d+)/);
    if (m) {
      totalInList = parseInt(m[2]);
      console.log(`\u{1F4CA} Total liste : ${totalInList} entreprises`);
    }
  } catch (_) {
  }
  while (hasMore) {
    ...
    console.log(`
Cartes visibles: ${currentCount} | Trait\xE9: ${totalProcessed} | Global: ${processedNames.size} / ${totalInList}`);
    const toProcess = [];
    for (const card of currentCards) {
      if (dryRun2 && totalProcessed + toProcess.length >= 10) break;
      let rawName = "";
      try {
        rawName = await card.locator(selectors2.companyName).first().textContent({ timeout: 1500 });
      } catch (_) {
      }
      if (!rawName?.trim()) {
        try {
          rawName = await card.locator("a").first().textContent({ timeout: 500 });
        } catch (_) {
        }
      }
      if (!rawName?.trim()) {
        try {
          rawName = (await card.innerText({ timeout: 500 })).split("\n")[0];
        } catch (_) {
        }
      }
    ...
    if (toProcess.length === 0) {
      ...
      console.log(`  \u2192 ${toProcess.length} nouvelles entreprises \xE0 traiter (${CONCURRENCY} workers)...`);
      await withPool(toProcess, CONCURRENCY, async (company) => {
        ...
        if (company.profileUrl) {
          ...
          try {
            ...
            if (turbo2) await tab.route("**/*.{png,jpg,jpeg,gif,svg,css,woff,woff2}", (route) => route.abort());
            await tab.goto(targetUrl, { waitUntil: "commit", timeout: turbo2 ? 12e3 : 25e3 });
            await waitRandom(turbo2 ? 150 : 800, turbo2 ? 400 : 1500);
            contacts = await extractContacts(tab, selectors2, turbo2);
          } catch (e) {
        ...
        for (let k = 0; k < 5; k++) {
          line += k < contacts.length ? `,${safeCSV(contacts[k].name)},${safeCSV(contacts[k].title)}` : `,N/A,N/A`;
        }
    ...
    if (processedNames.size >= totalInList) {
      console.log(`\u2705 Toutes les ${totalInList} entreprises trait\xE9es.`);
      hasMore = false;
      break;
    }
    const lastCard = currentCards[currentCards.length - 1];
    const lastCardText = await lastCard?.innerText({ timeout: 1e3 }).catch(() => "") || "";
    ...
    try {
      await page.waitForFunction(([sel, prev]) => {
        const all3 = document.querySelectorAll(sel);
        if (all3.length === 0) return false;
        return all3[all3.length - 1].innerText !== prev;
      }, [selectors2.companyCard, lastCardText], { timeout: turbo2 ? 6e3 : 15e3 });
      noNewItemsIter = 0;
      console.log(`   \u2192 Nouvelles cartes charg\xE9es !`);
    } catch (_) {
      noNewItemsIter++;
    ...
    if (noNewItemsIter >= 15) {
      console.log("\u2705 Fin de liste confirm\xE9e.");
      hasMore = false;
    }
    await waitRandom(turbo2 ? 100 : 600, turbo2 ? 250 : 1e3);
  }
  console.log(`
\u2705 TERMINE. Total: ${totalProcessed} entreprises.`);
}
async function processCompaniesGodMode(page, token, baseUrl, listId, dryRun2 = false, listName = "DEFAULT_LIST") {
  console.log(`
\u{1F31F} ETAPE 4 : EXTRACTION GOD MODE (API) - Liste: ${listName} (ID: ${listId})`);
  const csvDir = "data/lists";
  if (!import_fs2.default.existsSync(csvDir)) import_fs2.default.mkdirSync(csvDir, { recursive: true });
  const csvFile = `${csvDir}/${listName}.csv`;
  const safeCSV = (str) => (str || "N/A").replace(/,/g, " ").replace(/\n/g, " ");
  ...
  const pageSize = 1e3;
  let allCompanies = [];
  console.log("\u{1F4E5} Interception de la liste d'entreprises (Veuillez scroller sur la page si \xE7a bloque)...");
  ...
  console.log(`\u{1F4CA} API confirme ${totalInList} entreprises au total dans cette liste.`);
  ...
  if (reqMethod === "POST" && reqBodyStr && allCompanies.length < totalInList) {
    try {
      ...
      } else if (reqBody.pageSize !== void 0) {
        reqBody.pageSize = 100;
      } else if (reqBody.size !== void 0) {
        reqBody.size = 100;
      }
      const maxPages = Math.ceil(totalInList / 100);
      console.log(`\u{1F504} R\xE9cup\xE9ration active via API rep\xE9r\xE9e, estimation: ${maxPages} pages...`);
      allCompanies = [];
      for (let p = 0; p < maxPages; p++) {
        if (reqBody.pageParams) reqBody.pageParams.page = p;
        else if (reqBody.pageId !== void 0) reqBody.pageId = p;
        else if (reqBody.page !== void 0) reqBody.page = p;
        const res = await axios_default.post(reqUrl, reqBody, {
          headers: {
            "Authorization": token,
            "Content-Type": "application/json",
        ...
        if (data && data.companies && data.companies.length > 0) {
          ...
          console.log(`   \u{1F4C4} Page API ${p + 1}/${maxPages} ok : +${data.companies.length} entreprises (Total: ${allCompanies.length}/${totalInList})`);
        } else {
          break;
        }
        await new Promise((r) => setTimeout(r, 600));
        if (allCompanies.length >= totalInList) break;
        if (dryRun2 && p >= 2) break;
      }
    } catch (err) {
      console.error("\u26A0\uFE0F Impossible de paginer activement :", err.message);
    }
  }
  if (dryRun2 && allCompanies.length > 10) {
    allCompanies = allCompanies.slice(0, 10);
  }
  const uniqueCompaniesMap = /* @__PURE__ */ new Map();
  for (const c of allCompanies) {
    if (c.companyUniqueId) uniqueCompaniesMap.set(c.companyUniqueId, c);
  }
  allCompanies = Array.from(uniqueCompaniesMap.values());
  console.log(`\u{1F680} ${allCompanies.length} profils d'entreprise uniques r\xE9cup\xE9r\xE9s \xE0 la vitesse de la lumi\xE8re.`);
  const processedNames = /* @__PURE__ */ new Set();
  if (import_fs2.default.existsSync(csvFile)) {
    const content = import_fs2.default.readFileSync(csvFile, "utf-8");
    for (const line of content.split("\n")) {
  ...
  console.log(`\u{1F4C2} ${processedNames.size} entreprises d\xE9j\xE0 dans le CSV. Il en reste ${companiesToProcess.length} \xE0 traiter.`);
  ...
  await withPool(companiesToProcess, CONCURRENCY, async (company) => {
    ...
    const cNumber = fields["companies.compNumber"];
    let addressParts = [];
    ...
    try {
      ...
      if (err.response && err.response.status === 401) {
        if (!isRefreshingToken) {
          ...
          console.log(`\u26A0\uFE0F Token expir\xE9 d\xE9tect\xE9 (401) pour ${cName}. Tentative de rafra\xEEchissement...`);
          try {
            const newToken = await interceptToken(page);
            if (newToken) {
              token = newToken;
              console.log("\u2705 Nouveau token obtenu avec succ\xE8s.");
            } else {
              console.error("\u274C Impossible de rafra\xEEchir le token.");
            }
          } catch (tokenErr) {
            console.error("\u274C Erreur pendant le rafra\xEEchissement du token :", tokenErr.message);
          } finally {
        ...
        try {
          ...
          console.log(`\u274C Echec d\xE9finitif API contacts pour ${company.companyUniqueId} apr\xE8s retry : ${retryErr.message}`);
        }
      } else {
    ...
    if (contactsData && (contactsData.contacts || contactsData.items)) {
      const techKw = ["directeur technique", "technical director", "cto", "r&d", "technique", "ing\xE9nieur", "engineer", "developpeur", "developer", "it", "informatique", "syst\xE8me", "reseau", "data", "lead"];
      ...
      let parsedContacts = rawContacts.map((ct) => {
        ...
        if (ct.enrichmentSummary) {
          ...
          if (ct.enrichmentSummary.lastAvailablePhones && ct.enrichmentSummary.lastAvailablePhones.length > 0) {
            title += ` | \u{1F4DE} ${ct.enrichmentSummary.lastAvailablePhones[0]}`;
          }
        }
        return {
    ...
    for (let k = 0; k < 5; k++) {
      line += k < contactsToSave.length ? `,${safeCSV(contactsToSave[k].name)},${safeCSV(contactsToSave[k].title)}` : `,N/A,N/A`;
    }
    import_fs2.default.appendFileSync(csvFile, line + "\n");
    totalProcessed++;
    if (totalProcessed % 50 === 0 || totalProcessed === companiesToProcess.length) {
      console.log(`   \u23F3 Progression : ${totalProcessed} / ${companiesToProcess.length} entreprises trait\xE9es.`);
    }
  });
  console.log(`
\u2705 TERMINE (GOD MODE). Total trait\xE9: ${totalProcessed} entreprises.`);
}
async function extractContacts(targetPage, selectors2, turbo2 = false) {
  let contacts = [];
  try {
    await Promise.race([
      targetPage.waitForSelector("h1", { timeout: turbo2 ? 4e3 : 8e3 }),
      targetPage.waitForSelector(selectors2.contactRow, { timeout: turbo2 ? 4e3 : 8e3 })
    ]).catch(() => {
    });
    ...
    if (await contactTab.count() > 0 && await contactTab.isVisible().catch(() => false)) {
      if (await targetPage.locator(selectors2.contactRow).count() === 0) {
        ...
        await waitRandom(turbo2 ? 250 : 1200, turbo2 ? 500 : 2e3);
      }
    }
    ...
    for (const row of rows) {
      ...
      try {
        cName = (await row.locator(selectors2.contactName).first().innerText({ timeout: 1e3 })).trim();
      } catch (_) {
      }
      try {
        cTitle = (await row.locator(selectors2.contactTitle).first().innerText({ timeout: 1e3 })).trim();
      } catch (_) {
      }
      if (cName !== "N/A") contacts.push({ name: cName, title: cTitle });
    }
    const techKw = ["directeur technique", "technical director", "cto", "r&d", "technique", "ing\xE9nieur", "engineer", "developpeur", "developer"];
    contacts.sort((a, b) => {
      const aS = techKw.some((k) => a.title.toLowerCase().includes(k)) ? 1 : 0;
      const bS = techKw.some((k) => b.title.toLowerCase().includes(k)) ? 1 : 0;
      return bS - aS;
...
async function replayErrors(page, selectors2, listName = "REPLAY_RECOVERED") {
  console.log(`ETAPE 5 : REPLAY DES ERREURS (MODE RATTRAPAGE) - Liste: ${listName}`);
  const errorFile = "errors.json";
  const csvDir = "data/lists";
  if (!import_fs3.default.existsSync(csvDir)) import_fs3.default.mkdirSync(csvDir, { recursive: true });
  const csvFile = `${csvDir}/${listName}.csv`;
  ...
  if (!import_fs3.default.existsSync(csvFile)) import_fs3.default.writeFileSync(csvFile, headers);
  for (const item of errors) {
    ...
    try {
      await page.goto(item.url, { waitUntil: "domcontentloaded", timeout: 3e4 });
...
var currentDir;
try {
  currentDir = import_path2.default.dirname((0, import_url2.fileURLToPath)(import_meta2.url));
} catch (e) {
...
var turbo = args.includes("--turbo");
var godmode = args.includes("--godmode");
...
(async () => {
  ...
  if (replayMode) {
    ...
    let listName = "LIST_" + (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").split("T")[0];
    if (args.includes("--list")) {
      const idx = args.indexOf("--list");
      if (idx !== -1 && args[idx + 1]) {
        listName = args[idx + 1];
    ...
    if (godmode) {
      ...
      if (!token) {
        console.error("Echec du God Mode : Impossible de r\xE9cup\xE9rer le token d'API.");
        process.exit(1);
      }
  ...
  if (godmode) {
    console.log(" Termin\xE9. Fermeture du navigateur (God Mode).");
    await browser.close();
    process.exit(0);
  } else {
    console.log(" Termin\xE9. Le navigateur reste ouvert pour inspection.");
